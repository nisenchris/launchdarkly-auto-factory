...
function render(template, context) {
  ...
  return template.replace(
    /\{\{(\w+)\}\}/g,
    (_m, key) => context[key] === void 0 ? "" : String(context[key])
  );
}
...
async function walkGraph(graph, vega, context) {
  const runs = [];
  const accumulatedTags = {};
  const ctx = { ...context };
  const visited = /* @__PURE__ */ new Set();
  let current = graph.rootConfigKey;
  while (current && !visited.has(current)) {
    ...
    const result = await vega.runNode({
      ...
      context: ctx,
      ...inbound?.handoff?.max_turns !== void 0 ? { maxTurns: inbound.handoff.max_turns } : {}
    });
...
function createVegaClient() {
  ...
  if (endpoint && token) {
    const transport = new GraphQLVegaTransport({
      endpoint,
      token,
      ...
      ...process.env.LD_PROJECT_SLUG ?? process.env.LD_PROJECT_KEY ? { projectSlug: process.env.LD_PROJECT_SLUG ?? process.env.LD_PROJECT_KEY } : {}
    });
